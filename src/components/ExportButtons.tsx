...
export default function ExportButtons({ cvData, theme = 'clean', language = 'en', previewElementId = 'cv-preview', className = '' }: ExportButtonsProps) {
...
Create your own CV at: ${window.location.origin}`;
  ...
  const exportOptions = [
    { 
      type: 'pdf' as const, 
      label: t('export.pdf'), 
      description: t('export.best_for_applications'),
      icon: (
    ...
    { 
      type: 'docx' as const, 
      label: t('export.word'), 
      description: t('export.editable_format'),
      icon: (
        <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div className={`space-y-4 ${className}`}>
      ...
      <div className="mt-6">
        ...
        >
          ...
          <span>{t('export.share_whatsapp')}</span>
        </button>
      </div>
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mt-4">
        <div className="flex items-start space-x-3">
          ...
          <div>
            <h4 className="font-medium text-blue-900">{t('export.tips')}</h4>
            <ul className="text-sm text-blue-800 mt-1 space-y-1">
              <li>• {t('export.best_for_applications')}</li>
              <li>• {t('export.editable_format')}</li>
              <li>• {t('export.ats_friendly')}</li>
            </ul>
          </div>
        </div>
      </div>
