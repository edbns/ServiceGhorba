  ...
  const prompts = React.useMemo(() => {
    if (type === 'cv') {
      ...
      if (format && serviceWorkerFormats.includes(format)) {
        ...
        return (language === 'fr' ? guidedCVPrompts_fr : basicWorkerPrompts);
      } else {
        // Use consolidated multilingual prompts
        return multilingualCVPrompts[language as 'en' | 'fr'] || multilingualCVPrompts.en;
      }
    } else {
      // For motivation letters, use simple prompts if simple style is selected
      return simpleStyle ? guidedMotivationPrompts : motivationLetterPrompts;
    }
  }, [type, format, language, simpleStyle]);
  ...
  useEffect(() => {
    ...
    if (type === 'cv' && initialData) {
      ...
      if (initialData.experience) {
          ...
          return `${exp.role} at ${exp.company} (${exp.dates})${exp.location ? ` in ${exp.location}` : ''}: ${exp.description}`;
        }).join('\n\n');
      }
